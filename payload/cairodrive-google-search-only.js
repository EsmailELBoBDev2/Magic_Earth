  ...
  try{
    ...
    for(const provider of ['gps','fused','network','passive']){
      try{
        ...
        const c={provider,time:Number(l.getTime()),latitude:Number(l.getLatitude()),longitude:Number(l.getLongitude()),accuracy:Number(l.getAccuracy()),bearing:l.hasBearing()?Number(l.getBearing()):NaN};
...
function getLocationBias(){
  const now=Date.now();if(__locationBiasCache&&now-__locationBiasCacheAt<300000)return __locationBiasCache;
  let best=null;
  if(Java.available){
    try{Java.perform(()=>{
      const ActivityThread=Java.use('android.app.ActivityThread'),app=ActivityThread.currentApplication();if(!app)return;
...
function handleGemDispatch(raw){
  ...
  if(req.class==='LandmarkStoreCollection'&&req.method==='addStoreCategoryId'&&req.args){
    ...
    if(Number.isFinite(categoryId)){
      const tid=Process.getCurrentThreadId(),now=Date.now(),prev=pendingCategoryByThread.get(tid);
...
const TRAFFIC_MAP_DISCOVERY_COOLDOWN_MS=30000;
...
function ensureTrafficMapTaskClass(){
  ...
  __trafficMapTaskClass=Java.registerClass({
    ...
    methods:{invoke(){
      ...
      try{if(job.kind==='clear')clearTrafficMapPathsOnGemThread(job.reason);else if(job.kind==='render')renderTrafficMapPathsOnGemThread(job.payload);}catch(e){log(`TRAFFIC_MAP_GEM_ERROR ${String(e)}`);}
      return null;
    }}
  });
...
function enqueueTrafficMapJob(kind,payloadOrReason){
  ...
  __trafficMapJobs.splice(0,__trafficMapJobs.length);
  __trafficMapJobs.push(kind==='render'?{kind,payload:payloadOrReason}:{kind,reason:String(payloadOrReason||'')});
  postTrafficMapJobToGemThread();return true;
}
function getRealTrafficPathCollection(){
  ...
  try{
    ...
    try{prefs.setTrafficVisibility(true);}catch(_){}
...
function createTrafficNativePath(segment,index){
  ...
  const Coordinates=Java.use('com.magiclane.sdk.core.Coordinates');
  const PathCls=Java.use('com.magiclane.sdk.core.Path');
  const list=ArrayList.$new();
  for(const c of segment.coords)list.add(Coordinates.$new(Number(c.latitude),Number(c.longitude)));
  let coordsList=null;
  try{const CoordinatesList=Java.use('com.magiclane.sdk.core.CoordinatesList');coordsList=CoordinatesList.$new(list);}catch(_){}
  let path=null;
  for(const arg of [coordsList,list]){
    if(!arg||path)continue;
    try{if(PathCls.produceWithCoords)path=PathCls.produceWithCoords(arg);}catch(_){}
    if(!path){try{const comp=PathCls.Companion.value;path=comp.produceWithCoords(arg);}catch(_){}}
  }
  if(!path){log('TRAFFIC_MAP_PATH_CREATE_FAILED failOpen=yes');return null;}
  const name=`__cairodrive_traffic_${__navGeneration}_${index}_${String(segment.speed).toLowerCase()}`;
  try{path.setName(name);}catch(_){try{path.name.value=name;}catch(__){}}
  return path;
}
function addTrafficNativePath(collection,path,speed){
  const Rgba=Java.use('com.magiclane.sdk.core.Rgba'),c=trafficColor(speed);
  const fill=Rgba.$new(c[0],c[1],c[2],c[3]);
  const border=Rgba.$new(Math.max(0,c[0]-35),Math.max(0,c[1]-35),Math.max(0,c[2]-35),Math.min(235,c[3]+10));
  try{collection.add(path,border,fill,0.30,1.55);return true;}catch(e){log(`TRAFFIC_MAP_ADD_ERROR ${String(e)}`);return false;}
}
function renderTrafficMapPathsOnGemThread(payload){
  ...
  if(payload.fingerprint===__trafficMapFingerprint)return;
  ...
  for(let i=0;i<payload.segments.length&&i<TRAFFIC_MAP_MAX_PATHS;i++){
    const seg=payload.segments[i],key=seg.key||trafficSegmentKey(seg);
  ...
  if(next.length>0)__trafficMapFingerprint=payload.fingerprint;
...
function updateTrafficMapVisualization(match){
  ...
  const segments=buildTrafficMapSegments(match),fingerprint=trafficMapFingerprint(segments);
  if(!segments.length){if(__trafficMapEntries.length||__trafficMapFingerprint)enqueueTrafficMapJob('clear','empty-traffic-segments');return;}
  if(fingerprint===__trafficMapFingerprint)return;
  const payload={segments,fingerprint};
  if(!__trafficMapSurface){__trafficMapPending=payload;discoverTrafficMapSurface('traffic-update');return;}
  enqueueTrafficMapJob('render',payload);
}
...
async function requestGoogleTrafficAdvice(snapshot,generation){
  ...
  try{
    ...
    updateTrafficMapVisualization(match);
    ...
    const ahead=run.startRouteDistanceM-snapshot.progressed,remainingAfter=snapshot.total-run.endRouteDistanceM,now=Date.now();
    if(ahead<100||ahead>3500||remainingAfter<500||now-__lastTrafficRoadblockAt<120000)return;
    const length=Math.min(900,Math.max(120,run.lengthM+80));
    if(invokeNavigationRoadBlock(length,ahead,'traffic'))__lastTrafficRoadblockAt=now;
